    ...
    if (filter) {
      ...
      if (filter.category && filter.type && state.storeFilter) {
        ...
        let nature = state.storeFilter.filter((data) => {
          return (
            data.val == filter.category &&
            data.type.val == filter.type
          )
        })
        ...
        // set nature options
        return (state.storeFilterNature = createFilterFormat(
          natureTemp,
        ))
      }
    }
...
const actions = {
  ...
  storeFilterGet({ commit }) {
    return new Promise((resolve, reject) => {
      ...
      // request
      Vue.http
        .get(process.env.BACKEND_URI + '/product/category')
        .then((response) => {
  ...
  storePopularGet({ commit }) {
    return new Promise((resolve, reject) => {
      ...
      // request
      Vue.http.get(process.env.BACKEND_URI + '/product/popular').then(
        (response) => {
