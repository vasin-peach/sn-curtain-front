...
const mutations = {}
const actions = {
  async uploadProfile({ commit }, data) {
    // check image data
    ...
    // call backend
    let resp = await Vue.http.post(
      process.env.BACKEND_URI + '/upload/profile',
      formData,
    )
  ...
  upload({}, data) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // ! Call
      try {
        const resp = await Vue.http.post(
          `${process.env.BACKEND_URI}/upload/${data.destination}`,
          form,
        )
        return resolve(resp)
      } catch (err) {
